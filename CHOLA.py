...
def vote_cand1():
    ...
    cursor = conn.cursor()
    sql_retrieve= 'SELECT `Candidate_Name`, `Votes` FROM `chola` WHERE Candidate_Name="Name_1";'
    ...
    for x in result:
        ...
        new_val_vote=x[1]+1
        print(x[1]+1)
        sql_add= "UPDATE chola SET Votes={} WHERE Candidate_Name='Name_1';".format(new_val_vote)
...
def vote_cand2():
    conn = mysql.connector.connect(user='root', password='', host='localhost', database='voting_result')
    cursor = conn.cursor()
    sql_retrieve= 'SELECT `Candidate_Name`, `Votes` FROM `chola` WHERE Candidate_Name="Name_2";'
    cursor.execute(sql_retrieve)
    result=cursor.fetchall()
    for x in result:
        x=list(x)
        new_val_vote=x[1]+1
        print(x[1]+1)
        sql_add= "UPDATE chola SET Votes={} WHERE Candidate_Name='Name_2';".format(new_val_vote)
        cursor.execute(sql_add)
        conn.commit()
    conn.close()
...
def vote_cand3():
    conn = mysql.connector.connect(user='root', password='', host='localhost', database='voting_result')
    cursor = conn.cursor()
    sql_retrieve= 'SELECT `Candidate_Name`, `Votes` FROM `chola` WHERE Candidate_Name="Name_3";'
    cursor.execute(sql_retrieve)
    result=cursor.fetchall()
    for x in result:
        x=list(x)
        new_val_vote=x[1]+1
        print(x[1]+1)
        sql_add= "UPDATE chola SET Votes={} WHERE Candidate_Name='Name_3';".format(new_val_vote)
        cursor.execute(sql_add)
        conn.commit()
    conn.close()
...
def vote_cand4():
    conn = mysql.connector.connect(user='root', password='', host='localhost', database='voting_result')
    cursor = conn.cursor()
    sql_retrieve= 'SELECT `Candidate_Name`, `Votes` FROM `chola` WHERE Candidate_Name="Name_4";'
    cursor.execute(sql_retrieve)
    result=cursor.fetchall()
    for x in result:
        x=list(x)
        new_val_vote=x[1]+1
        print(x[1]+1)
        sql_add= "UPDATE chola SET Votes={} WHERE Candidate_Name='Name_4';".format(new_val_vote)
        cursor.execute(sql_add)
        conn.commit()
    conn.close()
...
def vote_cand5():
    conn = mysql.connector.connect(user='root', password='', host='localhost', database='voting_result')
    cursor = conn.cursor()
    sql_retrieve= 'SELECT `Candidate_Name`, `Votes` FROM `chola` WHERE Candidate_Name="Name_5";'
    cursor.execute(sql_retrieve)
    result=cursor.fetchall()
    for x in result:
        x=list(x)
        new_val_vote=x[1]+1
        print(x[1]+1)
        sql_add= "UPDATE chola SET Votes={} WHERE Candidate_Name='Name_5';".format(new_val_vote)
        cursor.execute(sql_add)
        conn.commit()
    conn.close()
...
def vote_cand6():
    conn = mysql.connector.connect(user='root', password='', host='localhost', database='voting_result')
    cursor = conn.cursor()
    sql_retrieve= 'SELECT `Candidate_Name`, `Votes` FROM `chola` WHERE Candidate_Name="Name_6";'
    cursor.execute(sql_retrieve)
    result=cursor.fetchall()
    for x in result:
        x=list(x)
        new_val_vote=x[1]+1
        print(x[1]+1)
        sql_add= "UPDATE chola SET Votes={} WHERE Candidate_Name='Name_6';".format(new_val_vote)
        cursor.execute(sql_add)
        conn.commit()
    conn.close()
...
window.title("VNPS Chola House (OASIS)")
